...
*/
const nickname = localStorage.getItem("nickname") || "Guest"; // added for track ###
console.log("Logged in as:", nickname); // added for track ###
...
// add here to track 1 (start)
/* ---------- Send score to Google Sheet ---------- */
async function updateUserScore(nickname, levelNum, score) {
  const url = "https://sheetdb.io/api/v1/asju5sk605b2u"; // replace with your Apps Script deployment URL
  const payload = {
    action: "updateScore",
    nickname,
    levelNum,
...
function showLevelEnd(){
  ...
  // added this for track ###
  updateUserScore(user.nickname, level + 1, score);
...
// added this for track ###
async function updateUserScore(nickname, levelNum, score) {
  const url = "https://sheetdb.io/api/v1/asju5sk605b2u"; // Apps Script Web App URL
  const payload = {
    action: "updateScore",
    nickname,
    levelNum,
  ...
  try {
    ...
    console.error("⚠️ Error updating score:", err);
  }
}
